...
-- this animation system is HEAVILY based on nutscripts system with partial recodes or changes to suit landis.
-- i claim no credit for this
-- check out NS at https://github.com/rebel1324/NutScript/

-- taken directly from impulse with permission from vin :), thanks vin

landis.Anim = landis.Anim or {}
landis.Anim.citizen_male = {
	normal = {
...
landis.Anim.citizen_female = {
	...
	glide = ACT_GLIDE,
	vehicle = landis.Anim.citizen_male.vehicle
}
landis.Anim.metrocop = {
	normal = {
	...
	shotgun = {
		[ACT_MP_STAND_IDLE] = {ACT_IDLE_SMG1, ACT_IDLE_RIFLE},
		[ACT_MP_CROUCH_IDLE] = {ACT_COVER_SMG1_LOW, ACT_COVER_SMG1_LOW},
		[ACT_MP_WALK] = {ACT_WALK_RIFLE, ACT_WALK_AIM_RIFLE},
		[ACT_MP_CROUCHWALK] = {ACT_WALK_CROUCH, ACT_WALK_CROUCH},
		[ACT_MP_RUN] = {ACT_RUN_RIFLE, ACT_RUN_AIM_RIFLE}
...
landis.Anim.overwatch = {
	normal = {
		[ACT_MP_STAND_IDLE] = {ACT_IDLE_ANGRY, ACT_IDLE_ANGRY},
		[ACT_MP_CROUCH_IDLE] = {"crouch_idled", "crouch_idled"},
		[ACT_MP_WALK] = {ACT_WALK, ACT_WALK},
		[ACT_MP_CROUCHWALK] = {ACT_WALK_CROUCH_RIFLE, ACT_WALK_CROUCH_RIFLE},
		[ACT_MP_RUN] = {"sprint_all", "sprint_all"}
	},
	pistol = {
		[ACT_MP_STAND_IDLE] = {"idle_unarmed", ACT_IDLE_ANGRY_SMG1},
		[ACT_MP_CROUCH_IDLE] = {"crouch_idle_rpg", "crouch_idled"},
		[ACT_MP_WALK] = {"walk_all", ACT_WALK_RIFLE},
		[ACT_MP_CROUCHWALK] = {ACT_WALK_CROUCH_RIFLE, ACT_WALK_CROUCH_RIFLE},
	...
	ar2 = {
		[ACT_MP_STAND_IDLE] = {ACT_IDLE_SMG1, ACT_IDLE_ANGRY_SMG1},
		[ACT_MP_CROUCH_IDLE] = {ACT_WALK_CROUCH_RIFLE, ACT_WALK_CROUCH_RIFLE},
		[ACT_MP_WALK] = {ACT_WALK_RIFLE, ACT_WALK_AIM_RIFLE_STIMULATED},
		[ACT_MP_CROUCHWALK] = {ACT_WALK_CROUCH_RIFLE, ACT_WALK_CROUCH_RIFLE},
		[ACT_MP_RUN] = {ACT_RUN_RIFLE, ACT_RUN_RIFLE}
	},
	shotgun = {
		...
		[ACT_MP_WALK] = {ACT_WALK_RIFLE, ACT_WALK_AIM_RIFLE},
		[ACT_MP_CROUCHWALK] = {ACT_WALK_CROUCH_RIFLE, ACT_WALK_CROUCH_RIFLE},
		[ACT_MP_RUN] = {ACT_RUN_RIFLE, ACT_RUN_RIFLE_RELAXED},
		attack = ACT_GESTURE_RANGE_ATTACK_SHOTGUN
	},
	grenade = {
...
function landis.Anim.SetModelClass(model, class)
	if not landis.Anim[class] then
...
function landis.Anim.GetModelClass(model)
...
landis.Anim.SetModelClass("models/police.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/hdpolice.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/civil_medic.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/hl2beta_police.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/retrocop.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/elite_police.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/policetrench.mdl", "metrocop")
landis.Anim.SetModelClass("models/dpfilms/metropolice/hl2concept.mdl", "metrocop")
landis.Anim.SetModelClass("models/combine_super_soldier.mdl", "overwatch")
landis.Anim.SetModelClass("models/combine_soldier_prisonGuard.mdl", "overwatch")
landis.Anim.SetModelClass("models/combine_soldier.mdl", "overwatch")
landis.Anim.SetModelClass("models/vortigaunt.mdl", "vort")
landis.Anim.SetModelClass("models/vortigaunt_blue.mdl", "vort")
landis.Anim.SetModelClass("models/vortigaunt_doctor.mdl", "vort")
landis.Anim.SetModelClass("models/vortigaunt_slave.mdl", "vort")
landis.Anim.SetModelClass("models/vortiblue1.mdl", "vort")
landis.Anim.SetModelClass("models/dpfilms/metropolice/police_fragger.mdl", "metrocop")
...
do
	...
	function meta:ForceSequence(sequence, callback, time, noFreeze)
		...
		if sequence and sequence > 0 then
			...
			self.landisSeqCallback = callback
			self.landisForceSeq = sequence
			...
			if time > 0 then
				timer.Create("landisSeq"..self:EntIndex(), time, 1, function()
	...
	function meta:leaveSequence()
		...
		self.landisForceSeq = nil

		if self.landisSeqCallback then
			self:landisSeqCallback()
		end
	end
	...
	if CLIENT then
		net.Receive("landisSeqSet", function()
			...
			if IsValid(ent) then
				if reset then
					ent.landisForceSeq = nil
					return
				end

				ent:SetCycle(0)
				ent:SetPlaybackRate(1)
				ent.landisForceSeq = sequence
			end
		end)
	end
end
...
local getModelClass = landis.Anim.GetModelClass
...
function GM:TranslateActivity(ply, act)
	...
	if class == "player" then
		if IsValid(weapon) and not ply.IsWeaponRaised(ply) and ply.OnGround(ply) then
			...
			local animTree = landis.Anim.player[holdType]
	...
	local animTree = landis.Anim[class]
...
function GM:CalcMainActivity(ply, velocity)
	...
	return seqIdeal, ply.landisForceSeq or oldSeqOverride or ply.CalcSeqOverride
end

function GM:DoAnimationEvent(ply, event, data)
	local model = ply:GetModel():lower()
	local class = landis.Anim.GetModelClass(model)
	...
	if class == "player" then
		...
		if IsValid(weapon) then
			...
			local animation = landis.Anim[class][holdType]
