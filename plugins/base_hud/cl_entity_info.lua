...
hook.Add( "HUDPaint", "drawEntityInformation", function()

	local ply = LocalPlayer()
	if not ply then return end

	local traceData = util.QuickTrace( ply:EyePos(), ply:EyeAngles():Forward() * 256, ply)
	local hitEnt

	if traceData.Hit then
		local e = traceData.Entity
		if (e.DisplayName and e.Description) or e:IsPlayer() then 
	...
	for v,k in ipairs( drawEnts ) do
		...
		if IsValid( k ) then
			...
			draw.DrawText( name, "landis-36-S-B", pos.x, pos.y-36-heightOffset, Color(col.r,col.g,col.b,alpha),TEXT_ALIGN_CENTER)
			draw.DrawText( desc, "landis-24-S", pos.x, pos.y-heightOffset, Color( 255, 255, 255, alpha ),TEXT_ALIGN_CENTER)
